...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="glass-card max-w-md w-full p-8">
        ...
        <form method="post" id="registerForm">
            ...
            <?php if ($smsVerifyEnabled): ?>
            <input type="text" name="sms_code" id="sms_code" placeholder="短信验证码" class="w-full px-4 py-3 rounded-full bg-white/40 border border-gray-200 mb-3" style="display: none;">
            <?php endif; ?>
    ...
    <script>
        ...
        sendSmsBtn.addEventListener('click', function() {
            ...
            if (!geetestValidated()) { alert('请先完成安全验证'); return; }
            <?php endif; ?>
            var body = 'phone=' + encodeURIComponent(phone) + '&type=register';
            <?php if (geetest_enabled()): ?>
            var gp = getGeetestParams();
            for (var k in gp) { body += '&' + k + '=' + encodeURIComponent(gp[k]); }
            <?php endif; ?>
            fetch('send_sms.php', {
                method: 'POST',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const timer = setInterval(() => {
                        smsCountdown--;
                        if (smsCountdown <= 0) { clearInterval(timer); sendSmsBtn.textContent = '获取验证码'; }
                        else { sendSmsBtn.textContent = smsCountdown + '秒后重试'; }
                    }, 1000);
        ...
        sendEmailBtn.addEventListener('click', function() {
            ...
            if (!geetestValidated()) { alert('请先完成安全验证'); return; }
            <?php endif; ?>
            var body = 'email=' + encodeURIComponent(email) + '&type=register';
            <?php if (geetest_enabled()): ?>
            var gp = getGeetestParams();
            for (var k in gp) { body += '&' + k + '=' + encodeURIComponent(gp[k]); }
            <?php endif; ?>
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const timer = setInterval(() => {
                        emailCountdown--;
                        if (emailCountdown <= 0) { clearInterval(timer); sendEmailBtn.textContent = '获取验证码'; }
                        else { sendEmailBtn.textContent = emailCountdown + '秒后重试'; }
                    }, 1000);
                } else { alert(data.message); }
            })
            .catch(err => alert('发送失败，请重试'));
